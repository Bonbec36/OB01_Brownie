...
if __name__ == "__main__":
    ...
    while True:
        ...
        k_visualisation_3D = get_input("Entrez la konstante de visualisation", default_value=5e-6, value_type=float, min_value=1e-20, max_value=1e20)
        ...
        for i in range(nb_groupement_h20_3D):
            ...
            vx_g = np.mean((v * np.sin(phi) * np.cos(theta)))
        ...
        particles.append(Particle3D(mass=10e-9*k_visualisation_3D, radius=1, x=wall_size_3D/2, y=wall_size_3D/2, z=wall_size_3D/2, vx=0, vy=0, vz=0))
